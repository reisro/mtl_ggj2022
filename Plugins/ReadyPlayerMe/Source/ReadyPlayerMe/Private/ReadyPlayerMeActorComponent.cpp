...
void UReadyPlayerMeActorComponent::LoadNewAvatar(FString url, UAnimBlueprint* animBlueprintInstance, USkeleton* skeleton, const FAvatarLoadFailed& onLoadFailed)
{
	if(!targetSkeleton && !skeleton)
	{
		onLoadFailed.ExecuteIfBound("No target skeleton set");
	}
	else if(!targetSkeleton)
	{
		targetSkeleton = skeleton;
	}
	if(!animBlueprint && !animBlueprintInstance)
	{
		UE_LOG(LogTemp, Warning, TEXT("No animation blueprint set") );
	}
	else if(!animBlueprint)
	{
		animBlueprint = animBlueprintInstance;
	}
	UrlShortcode = url;
...
void UReadyPlayerMeActorComponent::OnSkeletalMeshLoaded(USkeletalMesh* skeletalMesh)
{
	...
	skeletalMeshComponent = UReadyPlayerMeFunctionLibrary::SetupSkeletalMeshComponent(this->GetOwner(), skeletalMesh, avatarMetadata, animBlueprint);
}
