...
public:
		...
		static USkeletalMeshComponent* SetupSkeletalMeshComponent(AActor* targetActor, USkeletalMesh* skeletalMesh, const FAvatarMetadata& avatarMetaData, UAnimBlueprint* animBlueprintInstance);

		UFUNCTION(BlueprintCallable, meta = (DisplayName = "Set Full Body Animation Blueprint"), Category = "Ready Player Me")
		static void SetAnimationBlueprint(USkeletalMeshComponent * skeletalMeshComponent, UAnimBlueprint* animInstance);
		...
		static FglTFRuntimeSkeletalMeshConfig GetglTFRuntimeSkeletalMeshConfig(const FAvatarMetadata& avatarMetaData, USkeleton* skeleton = nullptr );
		...
		static AvatarBodyType GetBodyTypeFromAsset(UglTFRuntimeAsset* gltfRuntimeAsset);
